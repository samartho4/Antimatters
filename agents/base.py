...
def create_artifact(
    ...
    if tool_context is not None:
        ...
        if "artifacts" not in tool_context.state:
            tool_context.state["artifacts"] = {}

        tool_context.state["artifacts"][artifact_id] = {
            "type": artifact_type.value,
            "name": name,
        ...
        tool_context.state[type_key] = artifact_id
...
def read_artifact(artifact_id: str, tool_context: Any = None) -> Optional[Dict[str, Any]]:
    ...
    Args:
        ...
        tool_context: Optional ToolContext for session state lookup (ADK pattern)
