...
def create_artifact(
    ...
    artifact_data = {
        ...
        "name": name,
        "version": 0,  # ADK web expects top-level version (0-indexed)
    ...
    try:
        ...
        db_result = artifact_service.create(
        ...
        logger.info(f"[create_artifact] Created DB record for {artifact_id}: {db_result.get('id') if db_result else 'None'}")
    except Exception as e:
        import logging
        logging.getLogger(__name__).error(f"[create_artifact] DB insert failed for {artifact_id}: {e}")
...
def update_artifact(artifact_id: str, updates: Dict[str, Any], tool_context: Any = None) -> Optional[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        tool_context: Optional ToolContext for session state update
    ...
    # Also update top-level version for ADK web compatibility (0-indexed)
    ...
    # Also update database for UI visibility
    try:
        from core.api.services import ArtifactService, get_db
        import json as json_module
        import logging
        logger = logging.getLogger(__name__)

        # Find database record by searching metadata for our artifact_id
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT id, metadata FROM artifacts")
        rows = cursor.fetchall()

        db_artifact_id = None
        for row in rows:
            try:
                meta = json_module.loads(row["metadata"]) if row["metadata"] else {}
                if meta.get("artifact_id") == artifact_id:
                    db_artifact_id = row["id"]
                    break
            except:
                continue
        conn.close()
        ...
        if db_artifact_id:
            ...
            logger.info(f"[update_artifact] Updated DB record {db_artifact_id} for {artifact_id}")
        else:
            logger.warning(f"[update_artifact] No DB record found for {artifact_id} (searched {len(rows)} records)")
    except Exception as e:
        import logging
        logging.getLogger(__name__).error(f"[update_artifact] DB update failed for {artifact_id}: {e}")

    # Update session state if tool_context provided
    if tool_context is not None:
        state_dict = None
        if hasattr(tool_context, 'state') and tool_context.state:
            state_dict = tool_context.state
        ...
        if state_dict and "artifacts" in state_dict and artifact_id in state_dict["artifacts"]:
            state_dict["artifacts"][artifact_id]["updated_at"] = artifact["metadata"]["updated_at"]
...
def update_ligand_status(
    ...
    - interactions: From analyze_interactions - list of dicts with hbonds, hydrophobic, aromatic
    """
    artifact = read_artifact(artifact_id)
    if not artifact:
        return None
    ...
    for result in content["ligand_results"]:
        if result["ligand_name"] == ligand_name:
            ...
            if interactions:
                # Store full interaction data from analyze_interactions
                result["interactions"] = interactions
                # Also extract interaction types for backward compatibility
                if isinstance(interactions, list) and len(interactions) > 0:
                    if isinstance(interactions[0], dict):
                        # Extract types from detailed interaction data
                        types = set()
                        for frame_data in interactions:
                            if frame_data.get("hbonds"):
                                types.add("h_bond")
                            if frame_data.get("hydrophobic"):
                                types.add("hydrophobic")
                            if frame_data.get("aromatic"):
                                types.add("aromatic")
                            if frame_data.get("charged"):
                                types.add("charged")
                        result["interaction_types"] = list(types)
                    else:
                        # Already a list of type strings
