...
def configure_gemini_with_retry():
    ...
    print(f"  - Backoff: {RETRY.initial_delay}s → {RETRY.max_delay}s (exponential base {RETRY.exponential_base})", flush=True)
    print(f"  - Retryable codes: {RETRY.retryable_codes}", flush=True)
    print(f"  - Default timeout: 180s (accommodates retries)", flush=True)
    print(f"  - Model fallback enabled: {RETRY.model_rotation_enabled}", flush=True)
    print(f"  - Switch model after {RETRY.model_switch_after_retries} consecutive 503s", flush=True)
    print(f"  ⚠️  Note: On 503 errors, system will auto-switch to fallback models (gemini-2.0-flash is most stable)", flush=True)
