...
class ArtifactManager:
    """Manages artifacts generated during agent runs."""
...
class ADKAgentRunner:
    ...
    async def run_with_events(
        ...
        local_runner = Runner(
            agent=root_agent,
            app_name="agui_core", # change it to antimatters
        ...
        try:
            ...
            try:
                    ...
                    if hasattr(event, 'content') and event.content:
                        for part in event.content.parts if hasattr(event.content, 'parts') else [event.content]:
                            ...
                            if hasattr(part, 'function_call') and part.function_call:
                                ...
                                if task_idx >= 0:
                                    self.artifact_manager.update_task_progress(
                                ...
                                if task_idx >= 0:
                                    ...
                                    yield encoder.encode({
                                        ...
                                        "patch": [
                                            ...
                                            {"op": "replace", "path": "/content/completed", "value": task_artifact["content"]["completed"]}
                                        ]
                                    })
                ...
                # Check if it was a simple sync/async mismatch
            ...
            print(f"Error in agent run: {error_trace}")
...
# there can be phrase while it thinks whcih tell about antimatters iteslf like physics that realtes to information, compuation and evolution.
    ...
    async def _create_artifact_from_result(self, run_id: str, tool_name: str, result: Any) -> Optional[Dict]:
        """Create appropriate artifact from tool result."""
...
async def root():
    ...
    return {
        ...
        "mcp_servers": ["docking", "ped", "chembl"], 
    }
...
async def list_artifacts(run_id: Optional[str] = None):
    ...
    artifacts = list(_agent_runner.artifact_manager.artifacts.values())
    if run_id:
